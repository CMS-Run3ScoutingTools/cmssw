...
ScoutingElectronBestTrack::ScoutingElectronBestTrack(const edm::ParameterSet& iConfig)
  : run3ScoutingElectronToken_(
      consumes< std::vector<Run3ScoutingElectron> >(
        iConfig.getParameter<edm::InputTag>("Run3ScoutingElectron")
      )
    ) {
...
void ScoutingElectronBestTrack::produce(edm::Event& iEvent, const edm::EventSetup& iSetup) {
  ...
  if(!run3ScoutingElectronHandle.isValid()) {
    ...
    edm::LogWarning("ScoutingElectronBestTrack") << 
  ...
  for (const auto& electron : *run3ScoutingElectronHandle) {
    ...
    for (unsigned int i = 0; i < electron.trkpt().size(); ++i) {
      ...
      if (electron.trkpt().at(i) < trackPtMin_.at(eta_idx)) continue;
      if (electron.trkchi2overndf().at(i) > trackChi2OverNdofMax_.at(eta_idx)) continue;

      const math::PtEtaPhiMLorentzVector gsftrack(electron.trkpt().at(i), electron.trketa().at(i), 
                                                  electron.trkphi().at(i), 0.0005);

      if (deltaPhi(cluster.phi(), gsftrack.phi()) > deltaPhiMax_.at(eta_idx)) continue;

      const double track_ediff = abs((cluster.energy() - gsftrack.energy()) / cluster.energy());
      if (track_ediff > relativeEnergyDifferenceMax_.at(eta_idx)) continue;
